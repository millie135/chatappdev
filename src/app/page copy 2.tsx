...
export default function Home() {
  ...
  const unsubscribe = auth.onAuthStateChanged(async (u) => {
    ...
    try {
      const userRef = doc(db, "users", u.uid);
      const localSessionId = localStorage.getItem("sessionId") || generateUUID();
      localStorage.setItem("sessionId", localSessionId);
      ...
      if (!userSnap.exists()) {
        ...
        await setDoc(userRef, { sessionId: localSessionId, createdAt: serverTimestamp() });
      } else {
        const data = userSnap.data();
        if (data?.sessionId && data.sessionId !== localSessionId) {
          throw new Error("Your account is already logged in on another device.");
        }
        // Only update sessionId if missing or different
        if (!data?.sessionId || data.sessionId !== localSessionId) {
...
useEffect(() => {
  ...
  const usersQuery =
    user.role === "Leader"
      ? collection(db, "users")
      : query(collection(db, "users"), where("role", "==", "Leader"));
  ...
  const unsubUsers = onSnapshot(
    ...
    (error) => console.warn("Users query error:", error.code)
  );
  unsubscribersRef.current.push(unsubUsers);
  ...
  const unsubGroups = onSnapshot(
    ...
    (snapshot) => {
      ...
      if (chatUser && chatUser.isGroup && !fetchedGroups.some((g) => g.id === chatUser.id)) {
        setChatUser(null);
      }
    },
    (error) => console.warn("Groups query error:", error.code)
  );
  unsubscribersRef.current.push(unsubGroups);
